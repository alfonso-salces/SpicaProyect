        ...
        await Usuario.findOne({where: {id: payload.id}}).then( async function(comprobante) {
            if(comprobante){
                if(comprobante.rol != 'admin' && comprobante.rol != 'redactor') {
                    ...
                    await Categoria.findOne({ where: { id: req.body.categoria_id } }).then(async function (category) {

                        if(!category){
                            fs.unlinkSync(path.join("./", process.env.urlImagen + "/" + req.file.filename));
                            res.status(404).json({'error': 'No puedes asignar una noticia a una categoría inexistente.'});
                        } else{
                            ...
                            if(ext == 'jpg' || ext == 'png' || ext == 'jpeg') {
                                await Noticia.create({
                                    ...
                                    await Noticia.findOne({ where: { titular: req.body.titular} }).then(function (noticia) {
                                        if(!fs.existsSync(path.join("./", process.env.urlImagen + "/noticias"))) fs.mkdirSync(path.join("./", process.env.urlImagen + "/noticias"));
                                        if(!fs.existsSync(path.join("./", process.env.urlImagen + "/noticias/") + noticia.id)) fs.mkdirSync(path.join("./", process.env.urlImagen + "/noticias/") + noticia.id);
                                        fs.renameSync(path.join("./", process.env.urlImagen + "/" + req.file.filename), path.join("./", process.env.urlImagen + "/noticias/" + noticia.id + "/" + req.file.filename));
                                    }),
                                    )
        ...
        await Usuario.findOne({where: {id: payload.id}}).then( async function(comprobante) {
            if(comprobante){
                if(comprobante.rol != 'admin' && comprobante.rol != 'redactor') {
                    ...
                    var imagenNoticia;
                    await Noticia.findOne({ where: { id: req.params.id } }).then(async function (noticia) {
                        if(noticia) {
                    ...
                    await Noticia.destroy({ where: {id: req.params.id}}).then(async function(rowDeleted) {
                        if(rowDeleted === 1){
                            try {
                                fs.unlinkSync(path.join("./", process.env.urlImagen + "/noticias/" + req.params.id + "/" + imagenNoticia));
                                fs.rmdirSync(path.join("./", process.env.urlImagen + "/noticias/" + req.params.id))
                              } catch (err) {
                                console.log(err);
                              }
                          console.log('Noticia eliminada correctamente.');
                          res.status(200).json({'success': 'Noticia eliminada correctamente.'});
                        } else {
                          console.log('No existe la noticia especificada.');
                          res.status(404).json({'success': 'No existe la noticia especificada.'});
                        }
                    }, function(err){
                        console.log(err);
...
newsController.getNews = async(req, res, next) => {
    noticias = await Noticia.findAll().then(async function(noti) {
        if(noti) {
            res.json(noti);
        } else {
...
newsController.getNewsPerCategoryName = async(req, res, next) => {
    let CategoriaCons;
    await Categoria.findOne({where: {nombre: req.body.name}}).then(async function(categoria){
        if(!categoria) {
        ...
        noticias = await Noticia.findAll({where: {categoria_id: CategoriaCons.id}}).then(async function(noti) {
            if(noti) {
                res.json(noti);
            } else {
                res.status(404).json({'error': 'No hay noticias en esa categoría.'});
            }
        }).catch(err => res.status(400).json(err.msg));
    }).catch(err => res.status(400).json({'error': 'Bad request.'}));
}
